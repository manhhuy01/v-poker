    ...
    <nav className="absolute top-0 left-0 w-full z-30 bg-gray-900/40 backdrop-blur-md border-b border-white/5 text-white px-4 py-2 flex items-center justify-between">
      <div className="flex items-center space-x-3 md:space-x-6">
        ...
        <div className="flex items-center space-x-1">
          <button 
            ...
            className="px-3 py-2 rounded-lg text-sm font-bold bg-white/10 text-white transition-all hover:bg-white/20 active:scale-95"
          >
            Phòng chờ
          </button>
          ...
          <button 
            onClick={onChatOpen}
            className="relative px-3 py-2 rounded-lg text-sm font-bold bg-white/10 text-white transition-all hover:bg-white/20 active:scale-95 flex items-center space-x-2"
          >
            <svg className="w-5 h-5 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
            </svg>
            <span className="hidden xs:inline-block">Trò chuyện</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-4 relative" ref={dropdownRef}>
        ...
        >
          ...
          <span className="text-sm font-bold tracking-tight hidden sm:inline-block">{currentUser?.userName}</span>
